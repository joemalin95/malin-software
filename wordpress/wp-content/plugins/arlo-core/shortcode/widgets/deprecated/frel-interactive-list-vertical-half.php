 ...
 * Frel Site Title
 */
class Frel_Interactive_List_Vertical_Half extends Widget_Base {

	public function get_name() {
		return 'frel-interactive-list-vertical-half';
	}

	public function get_title() {
		return __( 'Interactive List Vertical Half', 'frenify-core' );
	}
	...
	protected function render() {
		...
		if($items_type == 'custom'){
			...
			foreach($list as $key => $reg){
				...
				$link			= '<h3><a href="'.$reg['r_project_url'].'">'.$reg['r_project_title'].$number.'</a></h3>';
				$projectList 	.= '<li><div class="list_inner"><img src="'.$reg['r_image']['url'].'" alt="" />'.$link.'</div></li>';
			}
		}
			...
			foreach ( $arlo_post_loop->posts as $key => $fn_post ) {
				...
				$number			= '<span class="number">'.$myKey.'</span>';
				$link			= '<h3><a href="'.$post_permalink.'">'.$post_title.$number.'</a></h3>';
				$projectList 	.= '<li><div class="list_inner"><img src="'.$post_image.'" alt="" />'.$link.'</div></li>';
			...
			foreach ( $arlo_post_loop->posts as $key => $fn_post ) {
				...
				$number			= '<span class="number">'.$myKey.'</span>';
				$link			= '<h3><a href="'.$post_permalink.'">'.$post_title.$number.'</a></h3>';
				$projectList 	.= '<li><div class="list_inner"><img src="'.$post_img.'" alt="" />'.$link.'</div></li>';

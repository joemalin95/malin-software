...
class Frel_Gallery extends Widget_Base {
	...
	protected function render() {
		...
		if($layout == 'collage'){
			...
			$firstReplacer	 = '<img src="'.ARLO_CORE_SHORTCODE_URL.'assets/img/thumb-7-3.jpg" alt="" />';
			$secondReplacer	 = '<img src="'.ARLO_CORE_SHORTCODE_URL.'assets/img/thumb-69-44.jpg" alt="" />';
			foreach ( $settings['gallery'] as $key => $image ) {
				if($key%3 == 0){
					$replacer 	= $firstReplacer;
				}
				$html .= '<li><div class="item">'.$replacer.'<div class="abs_img '.$l__child.'" data-src="'.$image['url'].'" data-bg-img="'.$image['url'].'"></div></div></li>';
			...
			foreach ( $settings['gallery'] as $key => $image ) {
				$fullURL	= $image['url'];
				$imageID	= $image['id'];
				$img		= wp_get_attachment_image_src( $imageID, 'arlo_fn_thumb-1000-9999');
				$imgURL		= $img[0];
				...
				$html			.= '<div class="swiper-container">';
					$html			.= '<div class="swiper-wrapper">';
						foreach ( $settings['gallery'] as $image ) {
							$html .= '<div class="swiper-slide"><div class="item"><div class="abs_img '.$l__child.'" data-bg-img="'.$image['url'].'" data-src="'.$image['url'].'"></div></div></div>';
						}
					$html 			.= '</div>';
					$html 			.= $pagination;
					$html 			.= $prevNextBtn;
			...
			foreach ( $settings['gallery'] as $key => $image ) {
				...
				$img		= wp_get_attachment_image_src( $imageID, 'arlo_fn_thumb-1000-1000');
				$imgURL		= $img[0];
				$imgHTML	= '<img src="'.$imgURL.'" alt="" />';
				$html 		.= '<li><div class="item '.$l__child.'" data-src="'.$fullURL.'"><div class="abs_img" data-bg-img="'.$imgURL.'"></div>'.$thumb.'</div></li>';
			}
